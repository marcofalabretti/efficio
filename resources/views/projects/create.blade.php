...
<div class="card" style="display:grid; gap:16px;">
    <div>
        <h1 style="margin:0; font-size:20px; font-weight:600;">Nuovo Progetto</h1>
        <p style="margin:6px 0 0; color:var(--efficio-muted)">Crea un nuovo progetto aziendale</p>
    </div>

    <form method="POST" action="{{ route('projects.store') }}" class="card" style="display:grid; gap:16px;">
        @csrf
            ...
            <div style="display:grid; gap:12px;">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" 
                              style="width:100%; padding:10px 12px; border-radius:8px; border:var(--border); background:var(--efficio-bg); color:var(--efficio-text); font-size:14px; resize:vertical;">{{ old('description') }}</textarea>
                    @error('description')
                        <p style="margin:4px 0 0; font-size:12px; color:#f87171;">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div style="display:grid; gap:12px;">
                <div>
                    ...
                    <select id="status" name="status" required 
                            style="width:100%; padding:10px 12px; border-radius:8px; border:var(--border); background:var(--efficio-bg); color:var(--efficio-text); font-size:14px;">
                        <option value="{{ App\Models\Project::STATUS_ACTIVE }}" {{ old('status') === App\Models\Project::STATUS_ACTIVE ? 'selected' : '' }}>Attivo</option>
                        <option value="{{ App\Models\Project::STATUS_COMPLETED }}" {{ old('status') === App\Models\Project::STATUS_COMPLETED ? 'selected' : '' }}>Completato</option>
                        <option value="{{ App\Models\Project::STATUS_SUSPENDED }}" {{ old('status') === App\Models\Project::STATUS_SUSPENDED ? 'selected' : '' }}>Sospeso</option>
                        <option value="{{ App\Models\Project::STATUS_CANCELLED }}" {{ old('status') === App\Models\Project::STATUS_CANCELLED ? 'selected' : '' }}>Cancellato</option>
                    </select>
                    @error('status')
                        <p style="margin:4px 0 0; font-size:12px; color:#f87171;">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="priority" name="priority" required 
                            style="width:100%; padding:10px 12px; border-radius:8px; border:var(--border); background:var(--efficio-bg); color:var(--efficio-text); font-size:14px;">
                        <option value="{{ App\Models\Project::PRIORITY_LOW }}" {{ old('priority') === App\Models\Project::PRIORITY_LOW ? 'selected' : '' }}>Bassa</option>
                        <option value="{{ App\Models\Project::PRIORITY_MEDIUM }}" {{ old('priority') === App\Models\Project::PRIORITY_MEDIUM ? 'selected' : '' }}>Media</option>
                        <option value="{{ App\Models\Project::PRIORITY_HIGH }}" {{ old('priority') === App\Models\Project::PRIORITY_HIGH ? 'selected' : '' }}>Alta</option>
                        <option value="{{ App\Models\Project::PRIORITY_CRITICAL }}" {{ old('priority') === App\Models\Project::PRIORITY_CRITICAL ? 'selected' : '' }}>Critica</option>
                    </select>
                    @error('priority')
                        <p style="margin:4px 0 0; font-size:12px; color:#f87171;">{{ $message }}</p>
                    @enderror
        ...
        <div style="display:flex; gap:12px; justify-content:flex-end; padding-top:16px; border-top:var(--border);">
            <a href="{{ route('projects.index') }}" class="icon-btn" style="padding:10px 20px; text-decoration:none; color:var(--efficio-text);">
                Annulla
            </a>
