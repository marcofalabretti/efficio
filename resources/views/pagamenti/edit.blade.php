...
<div style="max-width: 800px; margin: 0 auto;">
    ...
    <div class="card" style="margin-bottom: 24px; padding: 24px;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
            ...
            <div>
                ...
                <h1 style="margin: 0; font-size: 28px; font-weight: 600; color: var(--efficio-text);">Modifica Pagamento</h1>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <form action="{{ route('pagamenti.update', $pagamento) }}" method="POST" style="padding: 24px;">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 24px;">
                    ...
                    <select name="fattura_id" id="fattura_id" required 
                        ...
                        @foreach($fatture as $fattura)
                            <option value="{{ $fattura->id }}" 
                                    {{ old('fattura_id', $pagamento->fattura_id) == $fattura->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="importo" id="importo" value="{{ old('importo', $pagamento->importo) }}" step="0.01" min="0" required
                           style="width: 100%; padding: 12px 16px; border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; background: rgba(255,255,255,0.05); color: var(--efficio-text); font-size: 14px; transition: all 0.2s ease;">
                    @error('importo')
                        <p style="margin: 8px 0 0; font-size: 12px; color: #ef4444;">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="data_pagamento" id="data_pagamento" value="{{ old('data_pagamento', $pagamento->data_pagamento->format('Y-m-d')) }}" required
                           style="width: 100%; padding: 12px 16px; border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; background: rgba(255,255,255,0.05); color: var(--efficio-text); font-size: 14px; transition: all 0.2s ease;">
                ...
                <div>
                    ...
                    <select name="metodo_pagamento" id="metodo_pagamento" required
                        ...
                        <option value="bonifico" {{ old('metodo_pagamento', $pagamento->metodo_pagamento) == 'bonifico' ? 'selected' : '' }}>Bonifico</option>
                        <option value="assegno" {{ old('metodo_pagamento', $pagamento->metodo_pagamento) == 'assegno' ? 'selected' : '' }}>Assegno</option>
                        <option value="carta" {{ old('metodo_pagamento', $pagamento->metodo_pagamento) == 'carta' ? 'selected' : '' }}>Carta di Credito</option>
                        <option value="contanti" {{ old('metodo_pagamento', $pagamento->metodo_pagamento) == 'contanti' ? 'selected' : '' }}>Contanti</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="stato" id="stato" required
                        ...
                        <option value="pendente" {{ old('stato', $pagamento->stato) == 'pendente' ? 'selected' : '' }}>Pendente</option>
                        <option value="confermato" {{ old('stato', $pagamento->stato) == 'confermato' ? 'selected' : '' }}>Confermato</option>
                ...
                <div>
                    ...
                    <input type="text" name="riferimento_pagamento" id="riferimento_pagamento" value="{{ old('riferimento_pagamento', $pagamento->riferimento_pagamento) }}"
            ...
            <div style="margin-bottom: 24px;">
                ...
                <textarea name="note" id="note" rows="4" placeholder="Note aggiuntive sul pagamento..."
                          style="width: 100%; padding: 12px 16px; border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; background: rgba(255,255,255,0.05); color: var(--efficio-text); font-size: 14px; transition: all 0.2s ease; resize: vertical;">{{ old('note', $pagamento->note) }}</textarea>
                @error('note')
                    <p style="margin: 8px 0 0; font-size: 12px; color: #ef4444;">{{ $message }}</p>
                @enderror
            </div>
